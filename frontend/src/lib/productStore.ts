...
let productStore: ProductRecord[] = [];
let initialized = false;

export function getStore() {
  return { products: productStore, initialized };
}

export function setProducts(products: ProductRecord[]) {
...
function generateRecord(index: number): ProductRecord {
  ...
  return {
    ...
    category: cat.slug, subcategory: subcat, brand, price, msrp,
    currency: 'USD', specifications: '{}', colors: '["Midnight Black","Starlight White"]',
    sizes: '[]', rating, reviewCount: rand(0, 5000), releaseYear: year,
    inStock: Math.random() > 0.05, isLimitedEdition: Math.random() < 0.08, isRare: Math.random() < 0.05,
    ...
    features,
    popularity: rand(1, 10000), imageIndex: 0,
...
export function ensureStoreInitialized() {
  ...
  for (let i = 0; i < count; i++) records.push(generateRecord(i));
  setProducts(records);
  console.log('[productStore] Initialized with ' + count + ' products');
}
